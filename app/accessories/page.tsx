    ...
    const fetchProducts = async () => {
      try {
        ...
        const response = await fetch(
          `/api/accessories?page=${currentPage}&limit=${limitRef.current}`
        );
        const data = await response.json();
      ...
      <div className="flex-grow max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            {priceRange?.lowest ? (
              <>
                Our accessories start from just $
                {priceRange.lowest.price.toFixed(2)}. Choose from our wide range
              ...
              <>
                Discover our collection of high-quality accessories to
                complement your eyewear.
              </>
            )}
          </p>
        </div>
