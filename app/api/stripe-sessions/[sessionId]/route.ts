...
export async function GET(req: NextRequest, context: any) {
  try {
    ...
    console.log(
      '[DEBUG] First search with cleanSessionId:',
      cleanSessionId,
      ', found:',
      order?._id
    );
    ...
    if (!order) {
      ...
      console.log(
        '[DEBUG] Second search with original sessionId:',
        sessionId,
        ', found:',
        order?._id
      );
    ...
    if (!order && sessionId.startsWith('cs_test_')) {
      ...
      console.log(
        '[DEBUG] Third search with testSessionId:',
        testSessionId,
        ', found:',
        order?._id
      );
    ...
    if (!order) {
      order = await Order.findOne({
        'paymentDetails.stripeSessionId': cleanSessionId,
      });
      console.log(
        '[DEBUG] Fourth search with paymentDetails.stripeSessionId:',
        cleanSessionId,
        ', found:',
        order?._id
      );
    ...
    if (!order) {
      ...
      console.log(
        '[DEBUG] Fifth search with paymentDetails.stripeSessionId:',
        sessionId,
        ', found:',
        order?._id
      );
    ...
    if (!order) {
      const allOrders = await Order.find(
        {},
        { stripeSessionId: 1, 'paymentDetails.stripeSessionId': 1 }
      );
      console.log(
        '[DEBUG] All orders with session IDs:',
        allOrders.map((o) => ({
          id: o._id,
    ...
    if (checkStock && order.paymentStatus === 'paid' && !order.stockReduced) {
      try {
        ...
        for (const item of order.items) {
          await updateProductStock(item.productId, item.quantity, item.color);
        }
