...
export async function GET(request: NextRequest) {
  try {
    ...
    const featuredContacts = await ContactLenses.find({ featured: true })
      .select(
        'name price thumbnail images description discount brand inStock slug rating featured'
      )
      .sort({ createdAt: -1 })
      .limit(6);
    ...
    if (featuredContacts.length === 0) {
      ...
      const topRatedContacts = await ContactLenses.find()
        .select(
          'name price images description discount brand inStock slug rating featured'
        )
        .sort({ rating: -1 })
        .limit(6);
