    ...
    if (event.type === 'checkout.session.completed') {
      ...
      try {
        ...
        if (session.payment_status === 'paid') {
          ...
          console.log(
            '[DEBUG] Payment not successful, skipping order processing'
          );
        }
      } catch (error) {
      ...
      try {
        ...
        if (order) {
          ...
          if (order.paymentStatus !== 'paid') {
            ...
            console.log(
              `[DEBUG] Order ${order._id} already marked as paid, skipping stock update`
            );
          }
        } else {
          // Find session associated with this payment intent
          console.log(
            '[DEBUG] No order found by payment intent, looking for session...'
          );
          ...
          if (sessions.data.length > 0) {
            console.log(
              `[DEBUG] Found session for payment intent: ${sessions.data[0].id}`
            );

            // Find order by session ID
            order = await Order.findOne({
              'paymentDetails.stripeSessionId': sessions.data[0].id,
            });
            ...
            if (order) {
              ...
              if (order.paymentStatus !== 'paid') {
                console.log(
                  `[DEBUG] Updating order ${order._id} status to paid`
                );
                order.paymentStatus = 'paid';
              ...
              console.log(
                `[DEBUG] No order found for session: ${sessions.data[0].id}, creating one...`
              );
              await handleCheckoutCompleted(sessions.data[0]);
            }
          } else {
            console.log(
              '[DEBUG] No session found for payment intent:',
              paymentIntent.id
            );
          }
        }
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  try {
    ...
    const statusForUpdate =
      paymentStatus === 'paid' ? ('paid' as const) : ('failed' as const);
    ...
    try {
      ...
      console.log(
        `Order ${result.orderId} (${result.orderNumber}) updated to ${statusForUpdate} status`
      );
    } catch (error: any) {
      // If order not found, create it
      if (error.message && error.message.includes('Order not found')) {
        console.log(
          `Order not found for session ${expandedSession.id}, creating it now...`
        );

        // Create order directly from the Stripe session
        const orderResult = await createOrderFromStripeSession(expandedSession);
        console.log(
          `Created order from Stripe session: ${orderResult.orderId} (${orderResult.orderNumber})`
        );
        ...
        console.log(
          `Order ${updateResult.orderId} (${updateResult.orderNumber}) updated to ${statusForUpdate} status`
        );
      } else {
...
async function createOrderFromStripeSession(
  ...
  try {
    ...
    const backupProductIds = sessionMetadata.productIds
      ? sessionMetadata.productIds.split(',')
      : [];
    ...
    const orderItems = lineItems.map((item, index) => {
      ...
      if (!productId) {
        console.warn(
          `⚠️ Warning: Could not resolve product ID for item: ${
    ...
    const order = new Order({
      ...
      deliveryStatus:
        session.payment_status === 'paid' ? 'ORDER_CONFIRMED' : 'ORDER_PLACED',
...
async function updateProductStockFromStripeOrder(order: any) {
  try {
    console.log(
      'Updating product stock quantities for Stripe order:',
      order._id
    );
    ...
    for (const item of order.items) {
      ...
      console.log(
        `Updating stock for product ${item.product}, reducing by ${item.quantity}`
      );

      try {
        // Use our utility function to update the product stock (pass color if available)
        ...
        console.log(
          `Successfully updated stock for ${updatedProduct.name}: ${updatedProduct.stockQuantity} remaining (in stock: ${updatedProduct.inStock})`
        );
      } catch (itemError) {
        console.error(
        ...
        try {
          ...
          console.log(
            `Fallback: Current stock for ${product.name}: ${product.stockQuantity}`
          );
          ...
          console.log(
            `Fallback: Updated stock for ${product.name}: ${product.stockQuantity} remaining`
          );
        } catch (fallbackError) {
          console.error(
            `Fallback stock update also failed for ${item.product}:`,
            fallbackError
          );
        }
      }
...
async function updateProductStockDirectly(productId: string, quantity: number) {
  try {
    ...
    if (result) {
      ...
      console.log(
        `Direct update failed, product may be out of stock or not found`
      );
    }
  } catch (error) {
