...
export default function ProtectedRoute({ children, resource, action = "read" }: ProtectedRouteProps) {
  ...
  if (
    status === "authenticated" &&
    (!resource || (session?.user && hasPermission(session.user.role, resource, action)))
  ) {
    return <>{children}</>
  }
